...
run_visualizer_app() {
    ...
    local simulator_id=$(xcrun simctl list devices available | grep "iPhone 17 (.*) (Booted)" | grep -o '[A-F0-9-]\{36\}' | head -1)
    
    if [ -z "$simulator_id" ]; then
        echo -e "${YELLOW}Starting iPhone 17 simulator...${NC}"
        xcrun simctl boot "iPhone 17" 2>/dev/null
        sleep 3
        simulator_id=$(xcrun simctl list devices available | grep "iPhone 17" | grep -o '[A-F0-9-]\{36\}' | head -1)
    fi
