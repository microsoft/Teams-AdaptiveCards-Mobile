...
public:
    ...
    StyledCollectionElement(
        CardElementType type, ContainerStyle style = ContainerStyle::None, std::optional<VerticalContentAlignment> alignment = std::nullopt);
...
std::shared_ptr<T> StyledCollectionElement::Deserialize(ParseContext& context, const Json::Value& value)
{
    ...
    collection->SetStyle(ParseUtil::GetEnumValue<ContainerStyle>(
...
template <typename T>
void StyledCollectionElement::GetResourceInformation(
