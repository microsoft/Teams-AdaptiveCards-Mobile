//
//  TestExpressionsTests.swift
//  TestExpressionsTests
//
//  Created by Rahul Pinjani on 8/1/25.
//
...
class TestExpressionsTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        context = try await EvaluationContext()
    }
    ...
    private func getExpressionTestCases() -> [ExpressionTestCase] {
        return [
            ...
            //ExpressionTestCase(expression: "\"abc\" + 123", expectedResult: "abc123.0"),
            ...
            //ExpressionTestCase(expression: "123 + BadFunction('abc')", evaluateShouldThrow: true),
            ExpressionTestCase(
...
//            ExpressionTestCase(expression: "round('abc')", evaluateShouldThrow: true),
//            ExpressionTestCase(expression: "ceil('abc')", evaluateShouldThrow: true),
//            ExpressionTestCase(expression: "floor('abc')", evaluateShouldThrow: true),
          ...
          //  ExpressionTestCase(expression: "length(32)", evaluateShouldThrow: true),
           ...
           // ExpressionTestCase(expression: "Date.format(\"abc\")", evaluateShouldThrow: true),
            ExpressionTestCase(
                expression: "Time.format(\"2024-01-10T07:29:00Z\")",
                expectedResult: formatTimeForTest("2024-01-10T07:29:00Z")
            ),
            //ExpressionTestCase(expression: "Time.format(\"abc\")", evaluateShouldThrow: true),
           ...
           // ExpressionTestCase(expression: "[1,2,3] / 5", evaluateShouldThrow: true),
            ...
            // Invalid Expressions
           // ExpressionTestCase(expression: "1 *", parseShouldThrow: true)
        ]
    }
...
//    func testBinding_throwsOnInvalidExpression() {
//        XCTAssertThrowsError(try {
//            let _ = try Binding(expressionString: "${1 + 2")
//        }()) { error in
//            // Verify it's the expected error type
//            XCTAssertTrue(error is ExpressionError || error is TokenizerError)
//        }
//    }
    ...
    func compareResults(expected: Any?, actual: Any?) -> Bool {
        ...
        if let expectedArray = expected as? [Any], let actualArray = actual as? [Any] {
            ...
            for (index, expectedElement) in expectedArray.enumerated() {
                if !compareResults(expected: expectedElement, actual: actualArray[index]) {
                    return false
                }
            }
            return true
