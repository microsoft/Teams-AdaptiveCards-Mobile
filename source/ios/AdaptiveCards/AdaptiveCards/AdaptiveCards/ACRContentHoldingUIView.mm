...
- (void)updateLayoutForSwiftKVO
{
    // This method is specifically for Swift KVO layout updates to avoid timing issues
    // Only call this when using Swift KVO to prevent unnecessary layout cycles
    
    static NSMutableSet *currentlyUpdatingViews = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        currentlyUpdatingViews = [NSMutableSet set];
    });
    
    // Prevent infinite recursion
    if ([currentlyUpdatingViews containsObject:self]) {
        return;
    }
    
    [currentlyUpdatingViews addObject:self];
    ...
    NSInteger depth = 0;
    const NSInteger maxDepth = 10; // Prevent infinite loops in complex hierarchies
    
    while (currentView && depth < maxDepth) {
        ...
        if ([currentView isKindOfClass:[UITableViewCell class]] || 
            [currentView isKindOfClass:[UICollectionViewCell class]]) {
            
            [currentView setNeedsLayout];
            
            // Find and notify the parent collection/table view
            UIView *collectionCandidate = currentView.superview;
            while (collectionCandidate && depth < maxDepth) {
                if ([collectionCandidate isKindOfClass:[UITableView class]]) {
                    UITableView *tableView = (UITableView *)collectionCandidate;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [tableView beginUpdates];
                        [tableView endUpdates];
                    });
                    break;
                } else if ([collectionCandidate isKindOfClass:[UICollectionView class]]) {
                    UICollectionView *collectionView = (UICollectionView *)collectionCandidate;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [collectionView.collectionViewLayout invalidateLayout];
                    });
                    break;
                }
                collectionCandidate = collectionCandidate.superview;
                depth++;
            }
            break;
        }
    ...
    // Force layout update on self
    [self setNeedsLayout];
    
    // Remove from recursion protection after a brief delay
    dispatch_async(dispatch_get_main_queue(), ^{
        [currentlyUpdatingViews removeObject:self];
    });
}
